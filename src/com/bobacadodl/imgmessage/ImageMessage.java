...
public class ImageMessage {
    ...
    private ChatColor[][] toChatColorArray(BufferedImage image, int height) {
        ...
        for (int x = 0; x < resized.getWidth(); x++) {
            for (int y = 0; y < resized.getHeight(); y++) {
                ...
                ChatColor closest = ChatColorPallete.matchColor(new Color(rgb, true));
                chatImg[x][y] = closest;
            }
        }
        return chatImg;
	...
	private boolean areIdentical(Color c1, Color c2) {
        return Math.abs(c1.getRed() - c2.getRed()) <= 90 &&
                Math.abs(c1.getGreen() - c2.getGreen()) <= 90 &&
                Math.abs(c1.getBlue() - c2.getBlue()) <= 90;
	...
	private ChatColor getClosestChatColor(Color color) {
        ...
        for (int i = 0; i < colors.length; i++) {
            if (ChatColorPallete.areIdentical(colors[i], color)) {
